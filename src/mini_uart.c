...
void uart_init() {
  ...
  AUX_REG_MAP->AUX_ENABLES = SET_BIT(0);
  AUX_REG_MAP->AUX_MU_IER  = CLR;
  AUX_REG_MAP->AUX_MU_LCR  = SET_REG(AUX_REG_MAP->AUX_MU_LCR, UART_8BIT_MODE, 0);
  // No RTS
  AUX_REG_MAP->AUX_MU_MCR  = CLR;

  AUX_REG_MAP->AUX_MU_BAUD = UART_BAUD_RATE;
  AUX_REG_MAP->AUX_MU_CNTL = SET_REG(AUX_REG_MAP->AUX_MU_CNTL, UART_CNTRL_CFG, 0);
